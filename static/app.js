// Minimal React-like app using plain JS to keep dependencies low
// This file implements simple UI for /tickets, /tickets/new, /tickets/:id
...
async function pageNew(){
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    const res = await fetch('/api/tickets',{method:'POST', headers:{'Content-Type':'application/json','Idempotency-Key': idk, 'Authorization': 'Token '+localStorage.getItem('token')}, body});
...
async function pageTicket(id){
  ...
  cform.addEventListener('submit', async (e)=>{
    ...
    const idk = crypto.randomUUID();
    const resp = await fetch('/api/tickets/'+id+'/comments',{method:'POST', headers:{'Content-Type':'application/json','Idempotency-Key':idk, 'Authorization':'Token '+localStorage.getItem('token')}, body});
...
async function pageLogin(){
  ...
  f.addEventListener('submit', async (e)=>{
    ...
    const r = await fetch('/api/auth/login',{method:'POST', headers:{'Content-Type':'application/json'}, body:b});
